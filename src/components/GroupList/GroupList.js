...
const ENDPOINT = 'https://parallel-chat-server1.herokuapp.com';
...
const GroupList = ({
    ...
    const fetchGroupName = async () => {
      var list = [];
      const apiCall = await fetch("https://parallel-chat-server1.herokuapp.com/api/groups", {
        method: 'GET',
      });
    ...
    const joinGroup = async (name, groupName) => {
      ...
      await fetch("https://parallel-chat-server1.herokuapp.com/api/users/" + name + "/joingroup", {
          method: 'POST',
          headers: {
    ...
    const leaveGroup = async (name, groupName) => {
      ...
      await fetch("https://peaceful-bastion-78839.herokuapp.com/api/users/" + name + "/leavegroup", {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        listing.map((group) => {
          ...
          if (list.includes(name)) {
            return (
              ...
              <
              p onClick = {
                () => onChangeGroupChat(name, group['groupName'])
              } > {
                group['groupName']
              } < /p> <
              button className = "leave"
              onClick = {
                () => leaveGroup(name, group['groupName'])
              } > Leave < /button> < /
              li > <
              /div>
            )
          ...
          return (
            ...
            onClick = {
              () => joinGroup(name, group['groupName'])
            } > Join < /button> < /
            li > <
            /div>
          )
