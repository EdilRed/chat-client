...
const Chat = ({
  ...
  const fetchOldMessage = async (groupname, name) => {
    ...
    const apiCall = await fetch("https://parallel-chat-server1.herokuapp.com/api/groups/" + groupname + "/message", {
      method: 'GET',
    });
    const apiCall2 = await apiCall.json()
    const apiCallLastTimeStamp = await fetch("https://parallel-chat-server1.herokuapp.com/api/userRecords/" + name, {
      method: 'GET',
    });
